...
class Asset:
    ...
    def is_available(self):
        """Checks if asset quantity is over 0, returning availability."""
...
class AssetManager:
    ...
    def create_asset(self, name, type, asset_quantity, availability):
        ...
        sql = "INSERT INTO assets (name, type, asset_quantity, availability) VALUES ( %s, %s, %s, %s)"
        val = (name, type, asset_quantity, availability)
        try:
            ...
            return Asset(new_id, name, type, asset_quantity, availability)
        except mysql.connector.Error as err:
...
# Create new assets
asset1 = asset_manager.create_asset("Laptop", "Hardware", 4, True)
asset2 = asset_manager.create_asset("Microsoft Office", "Software", 5, True)

# Get all assets
assets = asset_manager.get_all_assets()
for asset in assets:
    print(f"Asset ID: {asset.asset_id}, Name: {asset.name}, Type: {asset.type}, Quantity: {asset.asset_quantity}, Availability: {asset.availability}")
